...
export function useAIAssistant(storyContent: string) {
  const isMobile = useIsMobile();
  const [isAIChatOpen, setIsAIChatOpen] = useState<boolean>(false);
  const [showAIFeatureHighlight, setShowAIFeatureHighlight] =
    useState<boolean>(true);
  const [hasInteractedWithAI, setHasInteractedWithAI] =
  ...
  const taleWeaverMessages = useMemo(
    () => [
  ...
  useEffect(() => {
    ...
    const hasSeenTaleWeaverIntro = localStorage.getItem(
      "hasSeenTaleWeaverIntro"
    );
    ...
    if (!isMobile && !isAIChatOpen && !showTaleWeaverIntro) {
      const messageInterval = setInterval(() => {
        const randomIndex = Math.floor(
          Math.random() * taleWeaverMessages.length
        );
  ...
  }, [isMobile, isAIChatOpen, showTaleWeaverIntro, taleWeaverMessages]);

  const dismissTaleWeaverIntro = useCallback(() => {
    setShowTaleWeaverIntro(false);
  ...
  const openAIAssistant = useCallback(
    ...
    [isMobile, dismissTaleWeaverIntro]
  );
  ...
  }, [isMobile, storyContent, hasInteractedWithAI, openAIAssistant]);
