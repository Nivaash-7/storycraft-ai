...
const genres = ["All Genres", "Fantasy", "Science Fiction", "Mystery", "Romance", "Horror", "Adventure"]
...
const Community = ({ stories, loading, error, setStories }: CommunityProps) => {
  ...
  useEffect(() => {
    ...
    if (selectedGenre !== "All Genres") {
      filtered = filtered.filter((story: Story) => story.genre === selectedGenre)
    }
    filtered = [...filtered].sort((a: Story, b: Story) => {
      switch (sortBy) {
        case "newest":
  ...
  const handleLike = async (storyId: string) => {
    ...
    try {
      ...
      setStories((prev: Story[]) => prev.map((story: Story) => (story._id === storyId ? updatedStory : story)))
  ...
  const toggleStoryExpansion = (storyId: string) => {
    saveScrollPosition(storyId)
    setExpandedStories((prev: Set<string>) => {
  ...
  const StoryCard = ({ story }: { story: Story }) => {
    ...
    const handleCommentSubmit = async () => {
      ...
      try {
        ...
        const updatedStory = await response.json()
        setStories((prev: Story[]) => prev.map((s: Story) => (s._id === story._id ? updatedStory : s)))
        setCommentInput("")
    ...
    return (
      <TooltipProvider>
        ...
        >
          <CardHeader className="space-y-4 pb-4 bg-background from-card to-muted/20">
            ...
            <div className="prose prose-lg max-w-none">
              ...
              >
                {isExpanded ? (
                  ...
                  <>
                    {(story.content || "No content available").slice(0, previewLength)}
              ...
              <div className="flex-1 w-full lg:w-auto">
                ...
                {searchTerm && (
                  <p className="text-sm text-muted-foreground mt-2 ml-1">
                    {filteredStories.length} result{filteredStories.length !== 1 ? "s" : ""} found
                  </p>
                )}
              </div>
                ...
                <div className="flex-1 lg:flex-none lg:w-44">
                  <Select value={selectedGenre} onValueChange={setSelectedGenre}>
                    ...
                    <SelectContent className="rounded-lg border-border/50 bg-card/95 backdrop-blur-sm">
                      {genres.map((genre) => (
                        <SelectItem key={genre} value={genre} className="rounded-lg cursor-pointer">
                          {genre}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div className="flex-1 lg:flex-none lg:w-44">
                  <Select value={sortBy} onValueChange={setSortBy}>
                    ...
                    <SelectContent className="rounded-lg border-border/50 bg-card/95 backdrop-blur-sm">
                      ...
                      <SelectItem value="word_count" className="rounded-lg cursor-pointer">
                        <div className="flex items-center gap-2">
